...
void set_bit(s21_decimal *decimal, unsigned position, bool value) {
  ...
  int *ptr = &decimal->bits[0] + position / PART_BITS;
  if (value == 1)
...
void print_decimal_binary(s21_decimal decimal) {
  ...
  for (int i = DATA_END; i >= MANTIS_1_PART; i--) {
    // Print bite (0 or 1)
...
s21_int256 shift_int256_left(s21_int256 decimal, int shift) {
  while (shift > 0){
    int b0 = get_bit(decimal.decimals[0], DATA_END);
    decimal.decimals[0] = shift_decimal_left(decimal.decimals[0], 1);
    decimal.decimals[1] = shift_decimal_left(decimal.decimals[1], 1);
    if (b0) {
      set_bit(&decimal.decimals[0], 0, 1);
    }
    --shift;
  }
...
s21_int256 shift_int256_right(s21_int256 decimal, int shift) {
  while (shift > 0){
    int b1 = get_bit(decimal.decimals[1], 0);
    decimal.decimals[0] = shift_decimal_right(decimal.decimals[0], 1);
    decimal.decimals[1] = shift_decimal_right(decimal.decimals[1], 1);
    if (b1) {
      set_bit(&decimal.decimals[0], 0, 1);
    }
    --shift;
