...
s21_decimal add_helper(s21_decimal a, s21_decimal b) {
  ...
  for (int i = 0; i <= MANTIS_END; i++) {      // Itterate through all bites
...
s21_decimal sub_helper(s21_decimal a, s21_decimal b) {
  s21_decimal result = {0, 0, 0,
                        s21_is_greater_module(a, b) ? a.bits[3] : b.bits[3]};
  unsigned borrow =
      0;  // Set thee result to the lowest exp, set the borrow var to zero

  for (int i = 0; i < 3; i++) {
    unsigned a_part = a.bits[i];  // Make a copy of a mantis
    unsigned b_part = b.bits[i];  // Make a copy of b mantis

    if (a_part <
        b_part + borrow) {  // If a mantis is less than b mantis + borrow
      result.bits[i] = (1LU << PART_BITS) + a_part - b_part - borrow;
      borrow = 1;  // Set the result to 2^32 in binary + a mantis - b mantis -
                   // borrow, set borrow 1
    } else {       // If a mantis is greater or equal than b mantis + borrow
      result.bits[i] = a_part - b_part - borrow;
      borrow =
