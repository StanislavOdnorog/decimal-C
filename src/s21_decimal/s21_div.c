...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (!result) {
    ...
    *result = s21_decimal_get_zero();
    ...
    res = s21_big_decimal_binary_division(value_1l, value_2l, &remainder);

    if (res.decimals[0].bits[3] != 0 ||
        !s21_binary_equal_zero(res.decimals[1])) {
      if (sign1 != sign2) {
...
int s21_div_handle(s21_big_decimal value_2l, s21_big_decimal res,
  ...
  if (s21_is_equal(tmp_res.decimals[0], s21_decimal_get_zerofive())) {
    if (!s21_binary_equal_zero(remainder.decimals[0]) ||
        !s21_binary_equal_zero(remainder.decimals[1])) {
...
int s21_div_calc_fractional(s21_big_decimal *res, s21_big_decimal value_2l,
  ...
  while ((!s21_binary_equal_zero((*remainder).decimals[0]) ||
          !s21_binary_equal_zero((*remainder).decimals[1])) &&
         power < 28) {
    ...
    number =
        s21_big_decimal_binary_multiplication(number, s21_decimal_get_ten());
    tmp_remainder = s21_big_decimal_binary_multiplication(
        tmp_remainder, s21_decimal_get_ten());
    tmp = s21_big_decimal_binary_division(tmp_remainder, value_2l,
                                          &tmp_remainder);
    number = s21_big_decimal_binary_addition(number, tmp);
...
s21_decimal s21_round_banking(s21_decimal integral, s21_decimal fractional) {
  ...
  if (s21_is_equal(fractional, zerofive)) {
    if (s21_decimal_even(integral)) {
      ...
      result = s21_binary_addition(integral, s21_decimal_get_one());
    }
